...
const PRODUCTION_CONFIG = {
    ...
    // Production environment detection (browser-safe)
    isProduction: () => {
        return window.location.hostname !== 'localhost' && 
               window.location.hostname !== '127.0.0.1';
    },
    
    // Supabase configuration for production (browser-safe)
    supabaseConfig: {
        url: 'https://ubokvxgxszhpzmjonuss.supabase.co',
        anonKey: 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InVib2t2eGd4c3pocHptam9udXNzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTg5NTE4MzEsImV4cCI6MjA3NDUyNzgzMX0.YourAnonKeyHere',
...
// Auto-initialize when script loads (localhost-safe)
(function() {
    
    // Check if we should run (avoid conflicts in localhost)
    const isLocalhost = window.location.hostname === 'localhost' || 
                       window.location.hostname === '127.0.0.1';
    
    if (isLocalhost) {
        
        // Simple fallback for localhost
        const activateLocalhostFallback = () => {
            const supabase = window.supabase || window.supabaseClient;
            if (!supabase) {
                setTimeout(activateLocalhostFallback, 100);
                return;
            }
            
            // Store original RPC if not already stored
            if (!window.originalSupabaseRpc) {
                window.originalSupabaseRpc = supabase.rpc.bind(supabase);
            }
            
            // Override commission function for localhost
                ...
                if (functionName === 'distribute_affiliate_commission') {
                    return Promise.resolve({
                        data: {
                            ...
                            admin_fallback: 0.00,
                            timestamp: new Date().toISOString(),
                            message: 'Commission distributed successfully (localhost)',
                            level_1: 500.00,
                            level_2: 100.00,
                            level_3: 100.00,
                            level_4: 100.00,
        ...
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', activateLocalhostFallback);
        } else {
            activateLocalhostFallback();
        }
        
    } else {
        // Production environment - full initialization
