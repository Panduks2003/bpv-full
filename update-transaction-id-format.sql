-- =====================================================
-- UPDATE TRANSACTION ID FORMAT - CLEAN & READABLE
-- =====================================================
-- This script updates the commission system to generate
-- clean, readable transaction IDs instead of messy ones
-- =====================================================

BEGIN;

-- =====================================================
-- UPDATE DATABASE FUNCTION FOR CLEAN TRANSACTION IDs
-- =====================================================

-- Update the distribute_affiliate_commission function to generate cleaner transaction IDs
...
DECLARE
    ...
    v_base_id VARCHAR(10);
BEGIN
    -- Generate a clean base ID for this commission batch
    v_base_id := LPAD((EXTRACT(EPOCH FROM NOW())::BIGINT % 10000)::TEXT, 4, '0');
    ...
    BEGIN
        ...
        FOR v_level IN 1..4 LOOP
            ...
            -- Generate clean transaction ID: COM-XXXX-LX format
            v_transaction_id := 'COM-' || v_base_id || '-L' || v_level;
        ...
        IF v_remaining_amount > 0 AND v_admin_id IS NOT NULL AND v_distributed_count < 4 THEN
            -- Generate clean admin transaction ID: COM-XXXX-ADM format
            v_transaction_id := 'COM-' || v_base_id || '-ADM';
        ...
        v_result := json_build_object(
            ...
            'base_transaction_id', 'COM-' || v_base_id,
            'message', CASE 
...
-- Add comment
COMMENT ON FUNCTION distribute_affiliate_commission IS 'Distributes ₹800 commission across 4 affiliate levels with clean transaction IDs. Admin fallback ONLY when hierarchy is incomplete.';

-- =====================================================
-- UPDATE TRIGGER FUNCTION FOR CLEAN TRANSACTION IDs
-- =====================================================

-- Update the trigger function to also generate clean transaction IDs
...
BEGIN
    ...
    BEGIN
        ...
        FOR v_level IN 1..4 LOOP
            ...
            -- Generate clean transaction ID: TRG-XXXX-LX format (to distinguish from manual)
            v_transaction_id := 'TRG-' || v_base_id || '-L' || v_level;
        ...
        IF v_remaining_pool > 0 AND v_admin_id IS NOT NULL THEN
            -- Generate clean admin transaction ID: TRG-XXXX-ADM format
            v_transaction_id := 'TRG-' || v_base_id || '-ADM';
...
-- =====================================================
-- SHOW SAMPLE OF NEW CLEAN TRANSACTION IDs
-- =====================================================

SELECT 
    '✅ TRANSACTION ID FORMAT UPDATED' as status,
    'New format: COM-XXXX-LX for manual, TRG-XXXX-LX for auto-trigger' as format,
    'Admin format: COM-XXXX-ADM or TRG-XXXX-ADM' as admin_format,
    'XXXX = last 4 digits of timestamp for uniqueness' as explanation;
...
-- =====================================================
SELECT 
    '🎉 CLEAN TRANSACTION ID SYSTEM DEPLOYED!' as final_status,
    'All new commissions will have clean, readable transaction IDs' as result,
    'Format: COM-1234-L1, COM-1234-L2, COM-1234-ADM' as example;
