-- =====================================================
-- FIX CUSTOMER LOGIN SYSTEM - CARD NO + PASSWORD ONLY
-- =====================================================
-- This script fixes customer creation to use simple email generation
-- since customers login with Customer ID + Password, not email
-- =====================================================
...
-- Create customer creation function with simple email generation
...
BEGIN
    ...
    BEGIN
        ...
        -- 4. HANDLE EMAIL - Use provided email or generate minimal auth email
        -- Since customers login with Customer ID + Password, email is just informational
        IF p_email IS NOT NULL AND TRIM(p_email) != '' THEN
            auth_email := TRIM(p_email);
        ELSE
            -- Generate minimal unique email for auth.users table requirement only
            auth_email := LOWER(p_customer_id) || '.' || extract(epoch from now())::bigint || '@system.local';
        END IF;
        ...
        -- 5. CREATE AUTH USER (required by Supabase, but customers won't use email to login)
        BEGIN
        ...
        EXCEPTION WHEN unique_violation THEN
            -- If email still conflicts, use customer ID + random number
            auth_email := LOWER(p_customer_id) || '.' || floor(random() * 1000000) || '@brightplanet.local';
            INSERT INTO auth.users (
                instance_id,
                id,
                aud,
        ...
        INSERT INTO profiles (
            id,
            name,
            email,  -- Store the provided email for information only
            phone,
            role,
            customer_id,
            state,
            ...
            new_customer_id,
            TRIM(p_name),
            NULLIF(TRIM(p_email), ''),  -- Store actual email or NULL if empty
            TRIM(p_mobile),
            'customer',
            p_customer_id,
            p_state,
        ...
        result := json_build_object(
            ...
            'login_method', 'Customer ID + Password',
            'customer_login_id', p_customer_id,
            'message', 'Customer created successfully. Login with Customer ID: ' || p_customer_id || ' and password.'
        );
