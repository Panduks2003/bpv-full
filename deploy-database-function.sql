-- =====================================================
-- DEPLOY COMMISSION FUNCTION TO DATABASE
-- =====================================================
-- Run this SQL in Supabase SQL Editor to fix commission system permanently
...
BEGIN
    ...
    FOR v_level IN 1..4 LOOP
        ...
        IF v_recipient_id IS NOT NULL THEN
            -- Create commission record for promoter
        ...
        ELSE
            ...
            -- Create commission record for admin (unclaimed)
            INSERT INTO affiliate_commissions (
                customer_id,
                initiator_promoter_id,
                recipient_id,
                ...
                v_amount,
                'credited',
                'ADMIN-' || EXTRACT(EPOCH FROM NOW())::BIGINT || '-' || v_level,
                'Unclaimed Level ' || v_level || ' Commission - ₹' || v_amount
            );
            
            -- Update admin wallet with unclaimed commission
            INSERT INTO admin_wallet (admin_id, balance, total_earned, commission_count, last_commission_at)
            VALUES (v_admin_id, v_amount, v_amount, 1, NOW())
            ON CONFLICT (admin_id) DO UPDATE SET
                balance = admin_wallet.balance + v_amount,
                total_earned = admin_wallet.total_earned + v_amount,
...
-- Test the function
SELECT 'Commission function deployed successfully!' as status;
