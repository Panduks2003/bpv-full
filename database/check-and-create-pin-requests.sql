-- =====================================================
-- CHECK AND CREATE PIN REQUESTS TABLE
-- =====================================================
-- This script checks if pin_requests table exists and creates it if needed
...
BEGIN
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'pin_requests table does not exist. Creating it now...';
        ...
        RAISE NOTICE 'pin_requests table created successfully!';
    ELSE
        RAISE NOTICE 'pin_requests table already exists.';
    END IF;
END $$;
...
SELECT 
    ...
    (SELECT COUNT(*) FROM pg_proc WHERE proname IN ('submit_pin_request', 'get_pin_requests')) as functions_count;
